...
class PoissonRegression:
    ...
    def fit(self, x, y):
        ...
        for j in range(160):
            ...
            for i in range(n):
                ...
                prediction = np.exp(self.theta.T @ xi)
                gradient = xi * (y[i] - prediction)
                dsum += gradient
            
            self.theta = self.theta + self.step_size * dsum
            # print(np.linalg.norm(prev_theta - self.theta))
    ...
    def predict(self, x):
        ...
        # *** START CODE HERE ***
        x = np.array(x)
        return (self.theta.T @ x.T).flatten()
        # *** END CODE HERE ***
